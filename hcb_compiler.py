...
def read_func(io, strings):
    ...
    if opcode_id not in opcodes:
        print(f'Warning: Unknown opcode: {opcode_id:x} at loc: {pos:x}')
        return None
...
def decompile(file_name):
    ...
    with open('strings.txt', 'wb') as strings_file:
        strings_file.write(b"\n".join(strings).decode('sjis', errors='replace').encode('utf8'))

def compile(file_name):
    script_data = marshal.loads(get_data('script.dat'))
    strings = get_data('strings.txt').decode('utf-8', errors='replace').encode('sjis', errors='replace').splitlines()
